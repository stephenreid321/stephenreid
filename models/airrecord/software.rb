class Software < Airrecord::Table
  ...
  def self.iframely
    Software.all(filter: "AND({Featured} = 1, {Description} = '')").each do |software|
      body = Faraday.get("https://iframe.ly/api/iframely?url=#{URI.encode_www_form_component(software['URL'])}&api_key=#{ENV['IFRAMELY_API_KEY']}").body
      json = JSON.parse(body.force_encoding('UTF-8'))
