class Post < Airrecord::Table
  ...
  def essay!
    post = self
    prompt = "Write a markdown-formatted essay based on this podcast transcript, as if by a listener of the podcast. Don't mention that you're a listener. Start with a # first level header, and then use ## second level headers for each topic covered.\n\n#{URI.open(post['Wizper txt'][0]['url']).read}"
